...
class Contactos(models.Model):
    ...
    def _check_phone(self):  # Función para comparar los campos de teléfono 
        for record in self:  # Iterar sobre todos los registros
            ...
            if record.phone and record.phone != '+52 1 00 0000 0000':  # Valida que tengamos numero para guardar y que sea diferen al que esta por defec
                existing_phone = self.env['res.partner'].search([('phone', '=', record.phone), ('id', '!=', record.id)], limit=1) # si encuentra de la el valor del numero
                _logger.info(f"El número que se va a registrar es {record.phone}, la ID del registro actual es {record.id} y la ID del contacto existente es {existing_phone.id if existing_phone else 'No existe'}")
...
class CrmLead(models.Model):
    ...
    @api.constrains('phone')
    def _check_pihone(self):  # Función para comparar los campos de teléfono
        for record in self:  # Iterar sobre todos los registros
            existing_phone = False # Declaracion de variable a comparar
            if record.phone and record.phone != '+52 1 00 0000 0000':  # Validar formato y que sea diferen al de defecto
                existing_phone = self.env['crm.lead'].search([('phone', '=', record.phone), ('id', '!=', record.id)], limit=1) #al ecnontrar le da valor


            #if existing_phone:  # Verificar duplicados al encontrar que si tenga valor
                #raise ValidationError("El número de teléfono ya está registrado en el CRM.")
